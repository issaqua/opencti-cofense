...
class CofenseConnector:
    ...
    def _create_observable(self, result: CofenseItem) -> Optional[Observation]:
        """
        ...
        create_indicator = self._config.Cofense.create_indicators
        if is_ip_indicator:
            create_indicator &= self._config.Cofense.create_ip_indicators

        sdo = sro = None
        if create_indicator:
            valid_until = (
                (date.today() + self._config.Cofense.ip_indicator_valid_until)
                if is_ip_indicator
                else None
            )
